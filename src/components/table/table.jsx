...
export default class Tables extends React.Component {
  ...
  handleSelectChange(value) {
    ...
    td3.innerHTML = '数量';
    if (value) {
      value.reduce((table, cur) => {
        const tr = table.insertRow();
        const td1 = tr.insertCell();
        const td2 = tr.insertCell();
        const td3 = tr.insertCell();
        td1.innerHTML = cur.group;
        td2.innerHTML = cur.grade + '-' + cur.name;
        td3.innerHTML = cur.data.length + '/' + cur.requireNum;
        td3.style.color = cur.pass ? '#13ce66' : '#ff4949';
        return table;
      }, table)
    }
    this.setState({ selectedData: table.outerHTML });
  }
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Export data={this.state.selectedData} />
      </div>
    );
  }
}